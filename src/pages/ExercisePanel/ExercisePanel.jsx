      ...
      <>
        ...
        <div id="exercise-panel">
          <iframe
            ...
            width="100%"
            height="315"
            src={exerciseObject.link}
            title={exerciseObject.name}
            allowFullScreen
          />
          <div id="exercise-panel__bottom">
            <div id="exercise-panel__data">
              <p className={`exercise-panel__data-name ${gender}`}>
                {exerciseObject.name}
              </p>
              <p id="exercise-panel__date-description-text">Description</p>
              <p className={`exercise-panel__data-description ${gender}`}>
            ...
            >
              <div
                className={`exercise-panel__timer-opener ${gender} ${
              ...
              >
                {timerRunning ? (
                  ...
                  <PauseIcon id="exercise-panel__timer-opener-pause-icon" />
                ) : (
                  <PlayIcon id="exercise-panel__timer-opener-play-icon" />
                )}
              </div>
            </Draggable>
            <div
              className={
                isToggled
                  ? "exercise-panel__timer show"
                  : "exercise-panel__timer"
              }
            >
              <div
                id="exercise-panel__timer-close-button"
                onClick={hideTimePanel}
                dangerouslySetInnerHTML={{ __html: "&times" }}
              />
              <button
                className={`exercise-panel__nav-button ${gender}`}
                onClick={() => handleNavigation("left")}
              >
                <ThinArrowRightIcon
                  className="exercise-panel__nav-button-icon"
                  id="exercise-panel__nav-button-left"
                />
              </button>
              <button
                className={`exercise-panel__timer-button ${gender}`}
                onClick={handleTimerButtonClick}
              >
                {timerRunning ? formatTime() : timerPaused ? "Paused" : "Start"}
              </button>
              <button
                className={`exercise-panel__nav-button ${gender}`}
              ...
              >
                <ThinArrowRightIcon
                  className="exercise-panel__nav-button-icon"
                  id="exercise-panel__nav-button-right"
                />
              </button>
            </div>
          </div>
